...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ status: 200, message: 'Record has been created', createHoliday })
    } catch (e) {
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        if (singleHoliday == null) return res.status(404).json({ status: 404, msg: `no record with id of ${id}` })
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ status: 200, message: `Record id ${id} successfully updated`, editHoliday })
    } catch (e) {
        checkingValidationError(e, req, res)
    }
})
